...
The image is configured with a local user called delphi. To start the container, use:

```
xhost + local:docker
docker run --rm -it -e DISPLAY --network host -v /tmp/.X11-unix:/tmp/.X11-unix -v ~/.Xauthority:/home/delphi/.Xauthority --user delphi gitlab-registry.cern.ch/delphi/deployment/delphi/al9_64 /bin/bash -l
```

This command will create a login shell for the DELPHI user. For Debian based distribution, you need to source the environment:
...
* dstana: dstana is the analysis framework of DELPHI. For historical reasons there are 3 different versions available. The default one and the one which should be used is the pro version. Only switch to a non default version is you know what you are doing.
...
* delgra: delgra is the event display of delphi.
