...
- One also need to know that some software components may need other *helper* software, so those services and modules also need to be present in the configuration file. However, most users will not need to worry about such details as there are utilities in CMSSW that will ensure that all service software is included, and the sequence of steps in the event processing chain is correct.
